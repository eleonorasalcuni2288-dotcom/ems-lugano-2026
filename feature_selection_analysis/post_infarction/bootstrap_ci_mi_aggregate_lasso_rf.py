"""
Bootstrap CI — Post-infarction complications, aggregate target,
LASSO + Random Forest (fair comparison vs DII+L1)
======================================================================
Extends bootstrap_ci_mi_aggregate.py's exact protocol to LASSO and RF,
matching the same fair-comparison treatment given to FRED-MD (see
bootstrap_ci_fredmd_lasso_rf.py). Same aggregate "any of 11
complications" target as bootstrap_ci_mi_aggregate.py, same K grid,
same downstream validation, B=15 (matching DII_L1's budget).
...
bootstrap_ci_mi_aggregate_results.csv (same column schema).
"""
...
B = 15  # matches DII_L1's budget in bootstrap_ci_mi_aggregate.py
...
if __name__ == "__main__":
    print("=" * 70)
    print("BOOTSTRAP CI — POST-INFARCTION AGGREGATE, LASSO + RF (B=%d)" % B)
    print("=" * 70)

    print("\n[1/2] Loading post-infarction data (aggregate target)...")
    ...
    results_df.to_csv('bootstrap_ci_mi_aggregate_lasso_rf_results.csv', index=False)
    print("\nSaved: bootstrap_ci_mi_aggregate_lasso_rf_results.csv")
